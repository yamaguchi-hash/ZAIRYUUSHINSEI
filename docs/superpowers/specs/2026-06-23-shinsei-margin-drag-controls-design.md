...
- `.role-banner`（申請人用→所属機関用／扶養者用の境目を示すバナー）は、在留資格カテゴリに応じて最大1つだけ実際にレンダリングされる（N型・V型は所属機関用バナー、R型は扶養者用バナーが表示され、同一の出力内に複数のロール境目バナーが同時に表示されることはない）。
...
- ロール境目ハンドル: `.role-banner`要素が存在する場合のみ、その直前に重ねて配置。ドラッグで`roleGapMargin`を変更。
