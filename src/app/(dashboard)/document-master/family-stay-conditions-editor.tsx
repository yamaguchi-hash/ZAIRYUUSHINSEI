 ...
 * 家族滞在｜在留資格認定証明書交付申請の自動チェックリスト（KazokuTairyuCoeChecklist）
 * が読む FamilyStayCoeConditions 形状を編集する。
...
import type { ConditionMatch, FamilyStayCoeConditions, Relationship, SupporterIncomeType, IdentityDocKey, FinancialProofKey } from "@/lib/kazoku-tairyu-coe-checklist";
import { RELATIONSHIP_LABELS, SUPPORTER_INCOME_TYPE_LABELS, IDENTITY_DOC_LABELS, FINANCIAL_PROOF_LABELS } from "@/lib/kazoku-tairyu-coe-checklist";
...
function MatchEditor({ value, onChange }: { value: ConditionMatch | undefined; onChange: (v: ConditionMatch | undefined) => void }) {
  ...
  return (
    <div className="space-y-0.5">
      ...
      <TriBool label="申請人のパスポート写しを添付する" value={v.attachApplicantPassportCopy} onChange={(attachApplicantPassportCopy) => patch({ attachApplicantPassportCopy })} />
    </div>
  );
}

export function FamilyStayConditionsEditor({
  value,
  onChange,
}: {
  ...
  const c: FamilyStayCoeConditions = (value ?? {}) as FamilyStayCoeConditions;
  const hasWhen = !!c.when;

  function patch(p: Partial<FamilyStayCoeConditions>) {
    const next = { ...c, ...p };
    const cleaned = Object.fromEntries(Object.entries(next).filter(([, val]) => val !== undefined));
