...
export function KazokuTairyuCoeChecklist({
  ...
  return (
    <div id="kazoku-tairyu-print-root" className="space-y-5">
      ...
      <style>{`
        @media print {
          ...
          #kazoku-tairyu-print-root, #kazoku-tairyu-print-root * { visibility: visible; }
          #kazoku-tairyu-print-root { position: absolute; left: 0; top: 0; width: 100%; }
        }
      `}</style>
      ...
      <div className="hidden print:block mb-4">
        ...
        <table className="w-full border-collapse text-[11px] mb-3">
          <tbody>
            ...
            <tr>
              ...
              <td className="border border-gray-300 px-2 py-1">家族滞在　在留資格認定証明書交付申請</td>
            </tr>
          </tbody>
        </table>
        ...
        <div className="bg-gray-50 border border-gray-200 border-l-4 border-l-blue-500 px-3 py-2 text-[11px] leading-relaxed mb-3">
          ...
          <p className="text-gray-500 text-[10px] mt-0.5">{EFFECTIVE_DATE_NOTE}（{LAST_REVIEWED_NOTE}）</p>
        </div>
      </div>
              ...
              return (
                <tr key={d.id} className={i % 2 === 1 ? "bg-gray-50" : ""}>
                  ...
                  <td className="border border-gray-300 px-2 py-1.5 font-semibold">
                    {d.name}
                    {optional && <span className="ml-1.5 text-[9px] font-normal border border-blue-400 text-blue-600 rounded px-1">任意・推奨</span>}
